...
var d=function(c){if(-1==b.indexOf(c))return"continue";var d=f(a[c]),e=c.replace("_onchange",""),g=document.getElementById(e);e=g.tagName.toLowerCase();1!=d.length||"value"!=d[0]&&"a"!=d[0]?0==d.length&&($input(g).onchange=function(){return a[c]()}):"input"==e||"textarea"==e?(d=g.getAttribute("type"),isNullOrUndefined(d)||"file"!=d.toLowerCase()?g.onchange=function(){var b=DOM.InputValueGetter.unifyGetValue(g);return a[c](b)}:g.onchange=function(){var b=$input(g).files;return a[c](b)}):"select"==
e?g.onchange=function(){var b=DOM.InputValueGetter.unifyGetValue(g);return a[c](b)}:TypeScript.logging.log("invalid tag name: "+g.tagName+"!","red")},e=0;for(c=c.methods;e<c.length;e++)d(c[e])}function f(a){if(isNullOrUndefined(a))return[];a=a.toString().replace(e,"");a=a.slice(a.indexOf("(")+1,a.indexOf(")")).match(h);return null===a?[]:a}d.findAllElement=c;d.hookEventHandles=function(d){for(var f=c("id"),e=TypeScript.Reflection.$typeof(d),g=0,h=e.methods;g<h.length;g++){var k=h[g];if(!Strings.Empty(k,
!1)){var u=k.replace("_click","").replace("_onclick","");b(d,f,k,k)||b(d,f,u,k)}}a(d,f,e)};var e=/((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg,h=/([^\s,]+)/g;d.parseFunctionArgumentNames=f})(d.EventHandles||(d.EventHandles={}))})(Internal||(Internal={}));var Framework;
