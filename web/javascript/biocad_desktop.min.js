...
c+"\n                                    </ul>\n                                "}else desktop.showToastMessage(e.info,f,"danger");$ts("#summary-info").display(d);isNullOrUndefined(h)||h()})});e.jumpToEnd()})})})};a.viewModel=function(a,c,b){a=JSON.stringify({guid:a,xref:c});apps.gcmodeller.sendPost($ts.url("@web_invoke_loadModel"),a).then(function(a){return __awaiter(this,void 0,void 0,function k(){return $jscomp.generator.createGenerator(k,function(b){desktop.parseMessage(a).then(function(b){desktop.parseResultFlag(a,
...
window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;this.uCanvas.height=window.innerHeight||document.documentElement.clientHeight||document.body.clientHeight;this.size=[this.uCanvas.width,this.uCanvas.height]};b.prototype.update=function(){var a=this,d=[a.f].concat(a.dots),f,g,k;a.uContext.clearRect(0,0,a.size[0],a.size[1]);var l=$jscomp.makeIterator(a.dots);for(f=l.next();!f.done;f=l.next()){var h=f.value;h.x+=h.xa;h.y+=h.ya;h.xa*=h.x>a.size[0]||0>h.x?-1:1;h.ya*=h.y>
a.size[1]||0>h.y?-1:1;a.uContext.fillRect(h.x-.5,h.y-.5,1,1);for(g=0;g<d.length;g++)if(f=d[g],h!==f&&null!==f.x&&null!==f.y){var m=h.x-f.x;var n=h.y-f.y;var p=m*m+n*n;p<f.max&&(f===a.f&&p>=f.max/2&&(h.x-=.03*m,h.y-=.03*n),k=(f.max-p)/f.max,a.uContext.beginPath(),a.uContext.lineWidth=k/2,a.uContext.strokeStyle="rgba("+a.setting.color+", "+(k+.2)+")",a.uContext.moveTo(h.x,h.y),a.uContext.lineTo(f.x,f.y),a.uContext.stroke())}d.splice(d.indexOf(h),1)}b.defaultCallback(function(){return a.update()})};
