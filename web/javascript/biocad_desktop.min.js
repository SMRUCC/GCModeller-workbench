...
"&lt;"));a=$ts("<li>").display(a);q++;n.appendElement(a)});$ts("#number-alert").show();$ts("#number_reactions").display(q.toString())};b.prototype.showGraph=function(a,b){var c={},d=[];b=$jscomp.makeIterator(b);for(var e=b.next();!e.done;e=b.next()){e=e.value;c[e.protein_id]={id:e.protein_id,name:e.protein_id,symbolSize:e.reactions.length+8,category:0};for(var n=$jscomp.makeIterator(e.reactions),k=n.next();!k.done;k=n.next()){k=k.value;k.entry in c||(c[k.entry]={id:k.entry,name:k.definition,symbolSize:8,
category:1});d.push({source:e.protein_id,target:k.entry});var l=k.equation;l=[].concat($jscomp.arrayFromIterable(l.Reactants)).concat(l.Products);l=$jscomp.makeIterator(l);for(var m=l.next();!m.done;m=l.next())m=m.value,m.ID in c||(c[m.ID]={id:m.ID,name:m.ID,symbolSize:8,category:2}),d.push({source:k.entry,target:m.ID})}}(new js_plot.graph_plot("container")).plot("Metabolic_graph@"+a,{nodes:$from(Object.keys(c)).Select(function(a){return c[a]}).ToArray(),links:d,categories:[{name:"Enzyme Protein"},
