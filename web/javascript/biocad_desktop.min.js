...
(function(c){var d=function(b,a,e){e=void 0===e?"Rplot_js":e;var f=c.echarts_ts.call(this,e)||this;f.title=b;f.subtitle=a;f.div=e;return f};$jscomp.inherits(d,c.echarts_ts);d.prototype.plot=function(b,a,e){e=void 0===e?null:e;var c=[];b&&a&&c.push({data:a,type:"line",areaStyle:{},smooth:!0,markPoint:{symbol:"none",symbolSize:1},name:b});if(e)for(b=$jscomp.makeIterator(e),a=b.next();!a.done;a=b.next())a=a.value,c.push({data:a.data,type:"line",areaStyle:{},smooth:!0,markPoint:{symbol:"none",symbolSize:1},
name:a.name});this.option={title:{text:this.title,subtext:this.subtitle,left:"center"},tooltip:{trigger:"item"},xAxis:{type:"value"},yAxis:{type:"value"},series:c,legend:{orient:"vertical",right:10,top:"center"}};this.show()};c.lineplot=d})(js_plot||(js_plot={}));
...
file:k,ssid:""};n.ssid=c.analysis_session.prototype.generateSsid.call(this,n);apps.gcmodeller.sendPost($ts.url("@web_invoke_clustering"),JSON.stringify(n)).then(function(a){return __awaiter(this,void 0,void 0,function m(){return $jscomp.generator.createGenerator(m,function(e){desktop.promiseAsyncCallback(a,function(a,e){a?(b.session_id=n.ssid,b.refresh_Rplot_onclick(),a=$ts.csv(e.info,!0).Objects().Take(10),$ts("#patterns-table").clear(),$ts.appendTable(a,"#patterns-table",null,{class:["table","table-sm"]})):
desktop.showToastMessage(e.info,"Run CMeans","danger")});e.jumpToEnd()})})})};d.prototype.refresh_Rplot_onclick=function(){var b=$ts.select.getOption("#colorSet");b={ssid:this.session_id,colorSet:b};desktop.loading();apps.gcmodeller.sendPost($ts.url("@web_invoke_Rplot"),JSON.stringify(b)).then(function(a){return __awaiter(this,void 0,void 0,function f(){return $jscomp.generator.createGenerator(f,function(b){desktop.promiseAsyncCallback(a,function(a,b){a?(a=b.info+"?refresh="+md5(desktop.now()),$ts("#Rplot-box").CType().href=
...
(function(c){var d=function(b){return c.analysis_session.apply(this,arguments)||this};$jscomp.inherits(d,c.analysis_session);d.prototype.init=function(){};d.prototype.button_open_click=function(){var b=$ts("#matrix-file").CType(),a=$ts("#busy-indicator");a.show();apps.gcmodeller.getFileOpen("Excel Matrix(*.csv)|*.csv").then(function(e){return __awaiter(this,void 0,void 0,function g(){var c;return $jscomp.generator.createGenerator(g,function(d){if(1==d.nextAddress)return c=b,d.yield(e,2);c.value=d.yieldResult;
...
function(a){a=JSON.parse(a||"[]");var c=Object.keys(a).length,d=$ts("#repository"),g={},k;for(k in a){g.$jscomp$loop$prop$key$37=k;g.$jscomp$loop$prop$metadata$38=a[g.$jscomp$loop$prop$key$37];var n=b.buildDbCard(g.$jscomp$loop$prop$key$37,g.$jscomp$loop$prop$metadata$38);d.appendElement(n);console.log(g.$jscomp$loop$prop$key$37);$ts("#"+g.$jscomp$loop$prop$key$37).onclick=function(a){return function(){apps.gcmodeller.openEnrichmentPage(a.$jscomp$loop$prop$key$37,a.$jscomp$loop$prop$metadata$38.name,
a.$jscomp$loop$prop$metadata$38.note)}}(g);$ts("#"+g.$jscomp$loop$prop$key$37+"-meta").onclick=function(a){return function(){desktop.loading();b.showMetadata(a.$jscomp$loop$prop$key$37,a.$jscomp$loop$prop$metadata$38)}}(g);g={$jscomp$loop$prop$key$37:g.$jscomp$loop$prop$key$37,$jscomp$loop$prop$metadata$38:g.$jscomp$loop$prop$metadata$38}}desktop.closeSpinner();desktop.showToastMessage("Found "+c+" database.","Enrichment Database Repository","info")};b.showMetadata=function(a,c){var d="",e=JSON.stringify({guid:a});
...
{$jscomp$loop$prop$name$40:c.$jscomp$loop$prop$name$40,$jscomp$loop$prop$model$41:c.$jscomp$loop$prop$model$41},e=d.next())c.$jscomp$loop$prop$model$41=e.value,c.$jscomp$loop$prop$name$40=c.$jscomp$loop$prop$model$41.name,e="#model-"+c.$jscomp$loop$prop$name$40,console.log(e),$ts(e).onclick=function(c){return function(){desktop.loading();console.log("view background model: "+c.$jscomp$loop$prop$name$40+"...");b.viewModel(a,c.$jscomp$loop$prop$name$40,c.$jscomp$loop$prop$model$41.info)}}(c)};c+='<span class="badge badge-primary">Protein Counts</span>: '+
...
(function(c){(function(c){var b=function(){var a=Bootstrap.apply(this,arguments)||this;a.port=$ts.location("service");a.pins=[];a.vectors=new Dictionary;return a};$jscomp.inherits(b,Bootstrap);b.prototype.init=function(){var a=$ts.url("@counts");$ts.get("http://localhost:"+this.port+a,function(a){for(var b=$ts("#module_list"),c=$jscomp.makeIterator(Object.keys(a.info)),d=c.next();!d.done;d=c.next())d=d.value,"Reactions"!=d&&b.appendElement($ts("<option>",{value:d}).display(d+" ["+a.info[d]+" molecules]"))})};
b.prototype.module_list_onchange=function(a){var b=this,c=$ts.url("@idset/?set="+encodeURIComponent(a[0]));b.cur_modu=a[0];c="http://localhost:"+this.port+c;$ts.get(c,function(a){var c=$ts("#molecules_list").clear();a=a.info;if(0<a.size){for(var d=$jscomp.makeIterator(a.set),e=d.next();!e.done;e=d.next())e=e.value,c.appendElement($ts("<option>",{value:e}).display(e));b.molecules_list_onchange([a.set[0]])}b.clear_onclick()})};b.prototype.clear_onclick=function(){this.pins=[];this.vectors=new Dictionary};
b.prototype.molecules_list_onchange=function(a){var b=a[0],c=this,d=this.cur_modu,k=this.vectors.Select(function(a){return{name:a.key,data:a.value}}).ToArray();this.reaction_bind=b;this.get_vector(a[0],function(a,c){(new js_plot.lineplot("Expression of "+b,"From module "+d,"container")).plot(b,c,k)});a=$ts.url("@reactions/?id="+b);$ts.get("http://localhost:"+this.port+a,function(a){a=a.info;var b=$ts("#reaction_list");Array.isArray(a)||(a=[a]);console.log("get related reaction id set:");console.log(a);
for(var d=$jscomp.makeIterator(a),e=d.next();!e.done;e=d.next())e=e.value,b.appendElement($ts("<option>",{value:e}).display(e));c.reaction_list_onchange(a[0])})};b.prototype.reaction_list_onchange=function(a){var b=a[0],c=[],d=this;this.get_vector(a[0],function(a,d){(new js_plot.lineplot("Expression of "+b,"From module reactions","reaction-container")).plot(b,d,c)},"Reactions");a=$ts.url("@graph/?id="+this.reaction_bind+"&rxn="+b);$ts.get("http://localhost:"+this.port+a,function(a){d.graph=a.info;
"string"==typeof d.graph.reactants&&(d.graph.reactants=[d.graph.reactants]);"string"==typeof d.graph.products&&(d.graph.products=[d.graph.products]);console.log(d.graph)})};b.prototype.view_mass_onclick=function(){var a=this.graph,b=[],c=[].concat($jscomp.arrayFromIterable(a.reactants)).concat(a.products);a={};for(var d=$jscomp.makeIterator(c),k=d.next();!k.done;a={$jscomp$loop$prop$id$43:a.$jscomp$loop$prop$id$43},k=d.next())a.$jscomp$loop$prop$id$43=k.value,this.get_vector(a.$jscomp$loop$prop$id$43,
function(a){return function(d,e){b.push({name:a.$jscomp$loop$prop$id$43,data:e});b.length==c.length&&(new js_plot.lineplot("Metabolite overlaps","Reaction Flux Viewer","container")).plot(null,null,b)}}(a))};b.prototype.get_vector=function(a,b,c){a=$ts.url("@vector/?m="+((void 0===c?null:c)||this.cur_modu)+"&id="+a);$ts.get("http://localhost:"+this.port+a,function(a){a=a.info;var c=a.vec,d=Array(a.size).fill().map(function(a,b){return[b,c[b]]});b(a.size,d)})};b.prototype.pin_onclick=function(){var a=
$ts.select.getOption("#molecules_list"),b=this;this.pins.push(a);this.get_vector(a,function(c,d){b.vectors.Add(a,d)})};$jscomp.global.Object.defineProperties(b.prototype,{appName:{configurable:!0,enumerable:!0,get:function(){return"vcell_viewer"}}});c.vcell_viewer=b})(c.viewers||(c.viewers={}))})(pages||(pages={}));
