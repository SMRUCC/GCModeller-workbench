...
(function(c){var d=function(b){b=void 0===b?"Rplot_js":b;this.option={};var a=window.echarts;$ts("#"+b).clear();this.myChart=a.init(document.getElementById(b),null,{renderer:"canvas",useDirtyRect:!1});this.option&&this.myChart.setOption(this.option)};d.prototype.hookChartResizeEvt=function(){window.addEventListener("resize",this.myChart.resize)};d.prototype.show=function(){this.option&&this.myChart.setOption(this.option)};c.echarts_ts=d})(js_plot||(js_plot={}));
...
(function(c){var d=function(b,a,e){e=void 0===e?"Rplot_js":e;var d=c.echarts_ts.call(this,e)||this;d.title=b;d.subtitle=a;d.div=e;return d};$jscomp.inherits(d,c.echarts_ts);d.prototype.plot=function(b,a){this.option={title:{text:this.title,subtext:this.subtitle,left:"center"},tooltip:{trigger:"item"},xAxis:{type:"value"},yAxis:{type:"value"},series:[{data:a,type:"line",areaStyle:{}}],legend:{orient:"vertical",show:!0}};this.show()};c.lineplot=d})(js_plot||(js_plot={}));
...
function(a){a=JSON.parse(a||"[]");var c=Object.keys(a).length,d=$ts("#repository"),g={},k;for(k in a){g.$jscomp$loop$prop$key$33=k;g.$jscomp$loop$prop$metadata$34=a[g.$jscomp$loop$prop$key$33];var n=b.buildDbCard(g.$jscomp$loop$prop$key$33,g.$jscomp$loop$prop$metadata$34);d.appendElement(n);console.log(g.$jscomp$loop$prop$key$33);$ts("#"+g.$jscomp$loop$prop$key$33).onclick=function(a){return function(){apps.gcmodeller.openEnrichmentPage(a.$jscomp$loop$prop$key$33,a.$jscomp$loop$prop$metadata$34.name,
a.$jscomp$loop$prop$metadata$34.note)}}(g);$ts("#"+g.$jscomp$loop$prop$key$33+"-meta").onclick=function(a){return function(){desktop.loading();b.showMetadata(a.$jscomp$loop$prop$key$33,a.$jscomp$loop$prop$metadata$34)}}(g);g={$jscomp$loop$prop$key$33:g.$jscomp$loop$prop$key$33,$jscomp$loop$prop$metadata$34:g.$jscomp$loop$prop$metadata$34}}desktop.closeSpinner();desktop.showToastMessage("Found "+c+" database.","Enrichment Database Repository","info")};b.showMetadata=function(a,c){var d="",e=JSON.stringify({guid:a});
...
{$jscomp$loop$prop$name$36:c.$jscomp$loop$prop$name$36,$jscomp$loop$prop$model$37:c.$jscomp$loop$prop$model$37},e=d.next())c.$jscomp$loop$prop$model$37=e.value,c.$jscomp$loop$prop$name$36=c.$jscomp$loop$prop$model$37.name,e="#model-"+c.$jscomp$loop$prop$name$36,console.log(e),$ts(e).onclick=function(c){return function(){desktop.loading();console.log("view background model: "+c.$jscomp$loop$prop$name$36+"...");b.viewModel(a,c.$jscomp$loop$prop$name$36,c.$jscomp$loop$prop$model$37.info)}}(c)};c+='<span class="badge badge-primary">Protein Counts</span>: '+
...
(function(c){(function(c){var b=function(a){return Bootstrap.apply(this,arguments)||this};$jscomp.inherits(b,Bootstrap);b.prototype.init=function(){$ts.get("@counts",function(a){for(var b=$ts("#module_list"),c=$jscomp.makeIterator(Object.keys(a.info)),d=c.next();!d.done;d=c.next())d=d.value,b.appendElement($ts("<option>",{value:d}).display(d+" ["+a.info[d]+" molecules]"))})};b.prototype.module_list_onchange=function(a){$ts.get("@idset/?set="+encodeURIComponent(a[0]),function(a){var b=$ts("#molecules_list").clear();
a=a.info;if(0<a.size){a=$jscomp.makeIterator(a.set);for(var c=a.next();!c.done;c=a.next())c=c.value,b.appendElement($ts("<option>",{value:c}).display(c))}})};b.prototype.molecules_list_onchange=function(a){var b=a[0],c=$ts.select.getOption("#module_list");console.log(a);console.log(c);$ts.get("@vector/?m="+c+"&id="+b,function(a){a=a.info;var d=a.vec;a=Array(a.size).fill().map(function(a,b){return[b,d[b]]});var e=new js_plot.lineplot("Expression of "+b,"From module "+c,"container");console.log(a);
e.plot(b,a)})};$jscomp.global.Object.defineProperties(b.prototype,{appName:{configurable:!0,enumerable:!0,get:function(){return"vcell_viewer"}}});c.vcell_viewer=b})(c.viewers||(c.viewers={}))})(pages||(pages={}));
