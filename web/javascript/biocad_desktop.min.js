...
function(a){a=JSON.parse(a||"[]");var c=Object.keys(a).length,d=$ts("#repository"),g={},k;for(k in a){g.$jscomp$loop$prop$key$32=k;g.$jscomp$loop$prop$metadata$33=a[g.$jscomp$loop$prop$key$32];var n=b.buildDbCard(g.$jscomp$loop$prop$key$32,g.$jscomp$loop$prop$metadata$33);d.appendElement(n);console.log(g.$jscomp$loop$prop$key$32);$ts("#"+g.$jscomp$loop$prop$key$32).onclick=function(a){return function(){apps.gcmodeller.openEnrichmentPage(a.$jscomp$loop$prop$key$32,a.$jscomp$loop$prop$metadata$33.name,
a.$jscomp$loop$prop$metadata$33.note)}}(g);$ts("#"+g.$jscomp$loop$prop$key$32+"-meta").onclick=function(a){return function(){desktop.loading();b.showMetadata(a.$jscomp$loop$prop$key$32,a.$jscomp$loop$prop$metadata$33)}}(g);g={$jscomp$loop$prop$key$32:g.$jscomp$loop$prop$key$32,$jscomp$loop$prop$metadata$33:g.$jscomp$loop$prop$metadata$33}}desktop.closeSpinner();desktop.showToastMessage("Found "+c+" database.","Enrichment Database Repository","info")};b.showMetadata=function(a,c){var d="",e=JSON.stringify({guid:a});
...
{$jscomp$loop$prop$name$35:c.$jscomp$loop$prop$name$35,$jscomp$loop$prop$model$36:c.$jscomp$loop$prop$model$36},e=d.next())c.$jscomp$loop$prop$model$36=e.value,c.$jscomp$loop$prop$name$35=c.$jscomp$loop$prop$model$36.name,e="#model-"+c.$jscomp$loop$prop$name$35,console.log(e),$ts(e).onclick=function(c){return function(){desktop.loading();console.log("view background model: "+c.$jscomp$loop$prop$name$35+"...");b.viewModel(a,c.$jscomp$loop$prop$name$35,c.$jscomp$loop$prop$model$36.info)}}(c)};c+='<span class="badge badge-primary">Protein Counts</span>: '+
...
(function(c){(function(c){var b=function(a){return Bootstrap.apply(this,arguments)||this};$jscomp.inherits(b,Bootstrap);b.prototype.init=function(){$ts.get("@counts",function(a){for(var b=$ts("#module_list"),c=$jscomp.makeIterator(Object.keys(a.info)),d=c.next();!d.done;d=c.next())d=d.value,b.appendElement($ts("<option>",{value:d}).display(d+" ["+a.info[d]+" molecules]"))})};b.prototype.module_list_onchange=function(a){$ts.get("@idset/?set="+encodeURIComponent(a[0]),function(a){console.log(a)})};
$jscomp.global.Object.defineProperties(b.prototype,{appName:{configurable:!0,enumerable:!0,get:function(){return"vcell_viewer"}}});c.vcell_viewer=b})(c.viewers||(c.viewers={}))})(pages||(pages={}));
